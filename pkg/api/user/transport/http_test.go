...
import (
	...
	"github.com/projeto-de-algoritmos/PD_BookingBR_Evaluation/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID PD_BookingBR_Evaluation.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr PD_BookingBR_Evaluation.User) (PD_BookingBR_Evaluation.User, error) {
					usr.ID = 1
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantResp != nil {
				response := new(PD_BookingBR_Evaluation.User)
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *PD_BookingBR_Evaluation.ListQuery, p PD_BookingBR_Evaluation.Pagination) ([]PD_BookingBR_Evaluation.User, error) {
					...
					return nil, PD_BookingBR_Evaluation.ErrGeneric
				},
			},
			wantStatus: http.StatusOK,
			wantResp: &listResponse{
				Users: []PD_BookingBR_Evaluation.User{
					{
						Base: PD_BookingBR_Evaluation.Base{
							ID:        10,
							CreatedAt: mock.TestTime(2001),
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (PD_BookingBR_Evaluation.User, error) {
					return PD_BookingBR_Evaluation.User{
						Role: &PD_BookingBR_Evaluation.Role{
							AccessLevel: PD_BookingBR_Evaluation.CompanyAdminRole,
						},
					}, nil
				},
			},
