...
func (s Service) EnforceRole(c echo.Context, r PD_BookingBR_Evaluation.AccessRole) error {
	return checkBool(!(c.Get("role").(PD_BookingBR_Evaluation.AccessRole) > r))
}
...
func (s Service) EnforceCompany(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, PD_BookingBR_Evaluation.CompanyAdminRole); err != nil {
		return err
	}
...
func (s Service) EnforceLocation(c echo.Context, ID int) error {
	...
	if err := s.EnforceRole(c, PD_BookingBR_Evaluation.LocationAdminRole); err != nil {
		return err
	}
...
func (s Service) isAdmin(c echo.Context) bool {
	return !(c.Get("role").(PD_BookingBR_Evaluation.AccessRole) > PD_BookingBR_Evaluation.AdminRole)
}

func (s Service) isCompanyAdmin(c echo.Context) bool {
	// Must query company ID in database for the given user
	return !(c.Get("role").(PD_BookingBR_Evaluation.AccessRole) > PD_BookingBR_Evaluation.CompanyAdminRole)
}
...
func (s Service) IsLowerRole(c echo.Context, r PD_BookingBR_Evaluation.AccessRole) error {
	return checkBool(c.Get("role").(PD_BookingBR_Evaluation.AccessRole) < r)
}
