...
func TestAuthenticate(t *testing.T) {
	...
	cases := []struct {
		name     string
		args     args
		wantData PD_BookingBR_Evaluation.AuthToken
		wantErr  bool
		udb      *mockdb.User
		jwt      *mock.JWT
		sec      *mock.Secure
	}{
		{
			...
			wantErr: true,
			udb: &mockdb.User{
				FindByUsernameFn: func(db orm.DB, user string) (PD_BookingBR_Evaluation.User, error) {
		...
		},
		{
			...
			udb: &mockdb.User{
				FindByUsernameFn: func(db orm.DB, user string) (PD_BookingBR_Evaluation.User, error) {
					return PD_BookingBR_Evaluation.User{Username: user}, nil
				},
			},
			sec: &mock.Secure{
		...
		},
		{
			...
			jwt: &mock.JWT{
				GenerateTokenFn: func(u PD_BookingBR_Evaluation.User) (string, error) {
...
func TestRefresh(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			wantErr: true,
			udb: &mockdb.User{
				FindByTokenFn: func(db orm.DB, token string) (PD_BookingBR_Evaluation.User, error) {
					return PD_BookingBR_Evaluation.User{}, PD_BookingBR_Evaluation.ErrGeneric
				},
			},
		},
	...
	}{
		{
			...
			rbac: &mock.RBAC{
				UserFn: func(echo.Context) PD_BookingBR_Evaluation.AuthUser {
					return PD_BookingBR_Evaluation.AuthUser{ID: 9}
				},
			},
			udb: &mockdb.User{
